...
Hermes can be an initial agent runtime, but it must sit behind a Jentera runtime adapter. Jentera—not the runtime—owns tenant isolation, business memory, permissions, approvals, integrations, audit history, billing, and the customer experience. This keeps the product portable across agent and model providers.
...
Build a reviewable business profile and memory
        ↓
Owner asks Jentera or Jentera proposes useful work
        ↓
...
```mermaid
flowchart LR
    UI[Web dashboard and onboarding] --> API[Jentera control-plane API]
...
- Exposes APIs for onboarding, Ask Jentera, Activity, approvals, and My Business.
...
- Places a narrow Jentera-owned agent runner in front of Hermes for workload identity,
...
- Presents stable Jentera tools such as `telegram.sendMessage`, `calendar.createBooking`, and `crm.updateLead`.
...
- **Decision taken.** Which option Jentera chose and the business reason, in a form that can be compared across runs. "Offered a consultation call" is derivable; "called the messaging tool" is not.
...
- **Owner corrections.** The delta between what Jentera proposed and what the owner actually approved. An approve or reject is a weak signal; an edit is the strongest one available, and it is the easiest to lose because it arrives as a modified draft rather than an event. Edits must be recorded as first-class events, with both versions retained.
...
- Let Jentera draft a customer reply from the business profile.
...
3. ask Jentera to respond to a realistic enquiry;
...
The owner must be able to understand what happened from the Home and Activity summaries without reading the Ask Jentera or Telegram transcript. Expanding a record must still reveal the exact source message, approved action, delivery result, and technical trace.
