...
| Turnstile on the sign-in doors | Code shipped 13 Sep on both sides, dormant until configured: the app build has no site key and the worker no secret | Owner creates the widget (hostnames jentera.ai and jentera.aisar.ai, Managed); site key goes into `app/.env.production` and the app is deployed; only then `wrangler secret put TURNSTILE_SECRET`. A signup from a fresh browser then shows the check and the worker logs no `[turnstile]` warnings |
