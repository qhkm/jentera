...
| **Telegram voice notes: vault bots, exhausted retries, the app mic** | Shipped 26 Sep for bots whose token the worker holds (5 of 6 connected): a voice note is downloaded in Telegram admission, transcribed with Whisper on Workers AI (`src/voice/transcribe.ts`, measurements in `docs/superpowers/plans/2026-09-26-telegram-voice-notes.md`), echoed as "🎤 …", and answered as text; the run's `trigger_ref` records `input: 'voice'`. A note it cannot make out, or one over 10 minutes, gets a one-line reply and no run. Three gaps remain: a vault-held bot still gets the stopgap reply until `aisar-vault` gains `POST /v1/telegram/<secretId>/file`; an intake whose download or transcription keeps failing goes to the DLQ after 8 tries with no reply; and the app composer has no mic | Five real voice notes in BM and Manglish are heard and answered on a sprite; the vault route ships and the vault-held bot is heard too; a failed-for-good voice intake gets a reply; the app composer gets a mic. Approvals stay on buttons: never inferred from speech |
