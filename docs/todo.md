...
| **Voice in the app composer** | Telegram voice notes are heard for every connected bot since 27 Sep (see Closed), but the app chat has no mic. The pieces exist: Whisper on Workers AI (`src/voice/transcribe.ts`), the 🎤 echo, and the voice note hint to the agent (`withVoiceNote`) | A mic in the app composer records a note, the transcript shows before the answer, and the run records `input: 'voice'`. Approvals stay on buttons: never inferred from speech |
