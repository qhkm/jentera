...
| Item | Why | Done when |
|---|---|---|
| **The person who asked in the app is pushed** | Shipped 24 Sep: app `a693e3d`, migration 069 applied, Worker version `15033209`. Before it, `notifications/work.ts` told only owners other than the requester, and all 37 businesses had one member, so approval and needs-you notifications reached nobody. Now the person who asked in the app is told of an approval (owners only), of a task needing them, and (new kind `work_finished`) of a work task that finished or failed two minutes or more after it was asked. Quick chat replies never push (14 days to 24 Sep: 252 app chat replies at a p50 of 15 s, against 7 `needs_input`, 31 completed and 14 failed work tasks). `deliverPendingPushes` sends at once rather than at the minute cron, and judges "due" by the database clock. Whether the owner is still watching is not checked: a phone put away can keep its WebSocket open, so a live socket would suppress exactly the push this exists for. As of 27 Sep: 3 devices subscribed and 7 pushes delivered since 24 Sep, 3 of them `work_needs_you`; no `approval_requested` or `work_finished` push has been sent yet | On a phone with notifications on, ask the app for something that needs approval: the push arrives within seconds and before the approval expires. Ask for a work task that takes over two minutes: "— done" arrives. `select kind, count(*) from notification where created_at > '2026-09-24 12:40+00' group by 1` shows `approval_requested`/`work_needs_you`/`work_finished` rows for solo businesses, and `push_outbox` shows them delivered with `attempts = 1` |
...
| Bookings v1 end to end | Plans 1–4 built on branch `bookings-v1` (not merged, nothing applied). Release order and checks are in each plan's "As built" | On Kitakod: apply 068 (`pnpm db:migrate:apps-bookings`), merge, deploy the app first, then `aisar-api`, then `pnpm deploy:sites`, set `TURNSTILE_SECRET` on sites, and flip `APPS_ENABLED` (Kitakod's id is already in both flag lists); set up a service, book from a phone, confirm from the notification, see the event in Google Calendar, cancel, see it removed |
...
| **Voice note to a vault-held bot** | Shipped 27 Sep: aisar-vault `2c773a6` (version `465a4ac7`) adds `POST /v1/telegram/<secretId>/file`, which fetches the file for a paired bot and returns the bytes alone; Worker `7c3b80c` downloads through it. The only vault-held bot is Parcel Tracker's @Jenterayed_bot, and the owner chose not to test it | A voice note to a vault-held bot shows the 🎤 echo and an answer; a `run` for that business has `trigger_ref->>'input' = 'voice'`, and `vault.audit` has a `secret.use` row with `detail->>'op' = 'download'` and outcome `ok` |
| **A quick reply that runs out of time** | Shipped 26 Sep: the app offers "Give it more time", which resends in deep mode (`0220a00`), and Telegram tells the owner to resend with `/deep` (`80c8cea`). No quick reply has timed out since | The next quick-mode timeout shows the button in the app, or the `/deep` line on Telegram, and the resend runs in deep mode |
| **Specialist hand-offs end to end (`ask_specialist`)** | Shipped 27 Sep in `2026.09.27-1` (bundle `30c344d`, Hermes `v2026.9.27`, app on jentera.ai; "As built" in `docs/plans/2026-09-26-specialist-handoff.md`). The switch ships empty (`HANDOFF_ENABLED = "true"`, `HANDOFF_BUSINESS_IDS = ""`): every sprite answered `available: false` after convergence. Switching it on for Kitakod (Task 13 step 7) is the owner's call | The first live hand-off on Kitakod with an approval in the middle (Task 13 step 8): "Who's working on this" reads "Chief of Staff + Finance and records" and lists that specialist's steps, the reply credits it, its approval is titled with its name, and the run's trace shows `agent.handoff` and never the brief. Move to Closed once it passes |
| Specialist usage during a hand-off | Kept in the runner's memory only, for the task's life (`HandoffEngine.usageOf`); a runner restart mid-task undercounts that task by its specialists' tokens, though the root run's own usage is still reported. A stopped or timed-out specialist's usage now settles for up to 12 s after its caller is answered, and the task's terminal record waits for it; but a *failed* specialist run carries no usage anywhere in Hermes (`run.failed` and its status record hold none), so its tokens are not counted — logged as `runner.handoff.usage_unmeasured`. The Worker charges its reserved ceiling only when a task reports no usage at all; it has no partial-usage rule, and none was invented | Accepted for the pilot; revisit if a restart mid-hand-off is seen on a live task, or if `usage_unmeasured` shows up often in sprite logs |
| A specialist run orphaned by a runner restart is never stopped | `HandoffEngine`'s state lives only in memory (final review M8, 27 Sep). After a restart, stopping the task cannot reach the old specialist run, which keeps going until its own Hermes budget runs out | `restore()` stops any Hermes run on a specialist profile that no task owns; or it stays accepted for the pilot and is revisited if seen live |
| A specialist's own output files | Land only if the specialist writes into the task's outputs folder, which the control-plane preamble points it at; a specialist that writes anywhere else leaves nothing the owner can open | Watch the first live hand-offs for a specialist reporting a file the owner cannot find |
| The approval window during a hand-off | Still 60 s (gap 4 below), the same clock as the root task's, so an owner slow to approve fails that specialist's part too. The hand-off's own time also keeps running while its approval waits: when it runs out first, the approval lapses on the runner as a deny, the Worker's timeout completes it, and an owner's late Approve reads as expired (27 Sep) | Revisited together with gap 4, in "Gaps in what is live" |
...
| Bukku staging token | Owner | Bukku writes (drafts behind owner approval) paused 25 Sep at Task 0 of `docs/superpowers/plans/2026-09-24-bukku-writes.md`; branch `bukku-writes` has Tasks 1, 6 and 7 done, unmerged. Resume at Task 0 with the token |
...
| **Setup failures, read by the stage they failed at** | Next time a provision or upgrade exhausts | This row used to say `STAGE:install` exhausts intermittently, and its query was wrong: the `STAGE:install` marker is printed before anything runs and `last_error` keeps only the last 500 characters of output, so `last_error like '%STAGE:install%'` counted almost any early failure, and "pinned Hermes dependencies…" is the patch script's success line. Read the stage the sprite reported instead. Over the 14 days to 27 Sep: `downloads` 524 (three sprites Fly could not wake, about 170 re-queues each, all ready since 24 Sep), `checks` 438 (release-wide failures 15–22 Sep, about 32 per business), `install` 39 (the 18 Sep tag incident, and one on 27 Sep that cleared). So nothing shows install flakiness, and [serving the bootstrap's bytes ourselves](plans/2026-09-17-prebuilt-bootstrap-bytes.md) was not built. `select coalesce(result->'setupProgress'->>'stage','(none)') as stage, count(*) from runtime_task where kind in ('provision','upgrade') and status in ('exhausted','failed') and updated_at > now() - interval '14 days' group by 1` |
...
- Time saved on Home and Activity. The "hours saved" tile and each receipt's
  "About N min saved" still show `minutes_saved`, a flat guess (3 per
  completed Telegram task, 2 per fact learned) that app work never sets.
  Summaries stopped claiming it on 26 Sep (`24b0494`); on 27 Sep the owner
  chose to leave the tile and receipts as they are.
...
- Move pre-vault Telegram bots into the vault (5 of 6 connected on 26 Sep), so
  one download path serves every bot.
...
- 27 Sep — Specialist hand-offs shipped with the switch empty: runtime `2026.09.27-1` (bundle `30c344d`, Hermes `v2026.9.27` on the fork's `release-lineage-2026.9.18`), fleet-verify 20/20, and every sprite answered `/v1/handoff/available` with `available: false`.
...
- 26 Sep — Web push on a real device: 3 rows in `push_subscription` and 7 pushes delivered since 24 Sep, none pending.
- 26 Sep — Daily and weekly summaries count only work: chat replies left out (`319d2bc`), cancelled work named as cancelled (`48ea89d`), earlier reports left out (`653a9c9`), and no minutes-saved claim (`24b0494`). The 25 Sep breakfast reminder, dismissed while its task kept asking, was set to completed by hand with an `operator.correction` event on its run.
- 26 Sep — A drafted reminder, once saved, completes the task that drafted it, and a reminder card no longer shows a second "Continue this task" (`b5d7d0a`).
- 26 Sep — Quick replies are told their five-minute limit and not to delegate; a quick-mode timeout offers "Give it more time" in the app (`0220a00`); Telegram says to resend with `/deep`, and a leading `/deep`, `/research` or `/quick` is stripped before Hermes sees it (`80c8cea`).
- 27 Sep — Telegram voice notes are heard: transcribed with Whisper on Workers AI, echoed once as "🎤 …", and answered as text (`9e371f8`…`82db37b`, review fixes `3b7f4ea`, `49d2b74`, `08197aa`, `4abb0f3`, `b9f603f`); a vault-held bot through aisar-vault's file route (vault `2c773a6`, Worker `7c3b80c`). First real note heard 27 Sep 00:07 on Kitakod. Plan and measurements: `docs/superpowers/plans/2026-09-26-telegram-voice-notes.md`.
- 27 Sep — aisar-vault had no git remote; it is now the private repo `qhkm/aisar-vault`, `main` pushed.
- 24 Sep — The three sprites Fly could not wake (My business, SEIDO Coffee Roasters, Stickoworld) converged on 2026.09.24-4 at 20:43–20:45 MYT, after the drift sweep had re-queued each upgrade about 170 times. Found 27 Sep.
- 27 Sep — A mic in the app composer: a recording (WebM, OGG or MP4) goes to `POST /api/voice/transcribe`, the same Whisper call Telegram uses, and its words land in the message box for the owner to check and send. Nothing is stored and nothing is sent by itself, so the run is the owner's typed message and is not marked `input: 'voice'`. Hidden in the demo, the native shell and browsers without `MediaRecorder`. Worker `d307e09`.
