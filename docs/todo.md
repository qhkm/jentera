...
| **Specialist hand-offs end to end (`ask_specialist`)** | Tasks 1–12 and Task 13 Step 1 of `docs/plans/2026-09-26-specialist-handoff-plan.md` built on branch `specialist-handoff-spec` ("As built" in `docs/plans/2026-09-26-specialist-handoff.md`), not merged; the switch ships empty (`HANDOFF_ENABLED = "true"`, `HANDOFF_BUSINESS_IDS = ""`). Steps 2–8 (push, app deploy, fork tag, pin, `ship-runtime.sh`, switch on, live check) are the owner's call | The first live hand-off on Kitakod with an approval in the middle (Task 13 step 8): "Who's working on this" reads "Chief of Staff + Finance and records" and lists that specialist's steps, the reply credits it, its approval is titled with its name, and the run's trace shows `agent.handoff` and never the brief. Move to Closed once it passes |
...
| Hermes pin bump that ships `ask_specialist`, and switching hand-offs on for Kitakod | Task 13 Steps 3–5 tag and pin a Hermes release carrying the fork's `jentera-ask-specialist` commits (`plugins/jentera`'s `ask_specialist`) and ship the runtime bundle carrying `runner/src/handoff.mjs`; Step 7 sets `HANDOFF_BUSINESS_IDS` to Kitakod's id — pending release | The release converges (`fleet-verify.sh` clean on every sprite) and `HANDOFF_BUSINESS_IDS` names Kitakod alone |
