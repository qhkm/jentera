...
| **Telegram voice notes vanish** | `parseUpdate` (`connectors/telegram.ts`) returns null for any message without `text`, deliberately silent so Telegram does not redeliver. A voice note, which Malaysian owners send constantly and often in BM, gets no reply and no trace. Workers AI is already bound as `AI`. Telegram's file URL carries the bot token, so the download belongs in the vault Worker behind a new allow-listed route, never in the main Worker | A voice note to the business bot is transcribed (Whisper on Workers AI), answered like typed text, and the run records that it came from audio. A voice note that cannot be transcribed gets a one-line reply asking for text, not silence. Then the same for a mic in the app composer. Approvals stay on buttons: never inferred from speech |
| **A solo owner is never told an approval or a result is waiting** | `notifications/work.ts` sends `approval_requested` and `work_needs_you` to owners `except` the requester, and returns 0 when there is no requester, so a business of one receives neither (by design: `notifications/recipients.ts` says the requester "is watching the answer land"). That holds on the web only while the tab stays open. An owner who asks and pockets the phone learns nothing, and with a 60 s approval window (next row) the approval has expired before they look | The requester gets a push for approval-needed and needs-you, solo owners included. For "finished", push only when the run's stream was no longer connected when it ended; decide that on the server, not in the service worker, because Chrome only allows a push without a visible notification while a window is focused, and Safari may cancel subscriptions that receive silent pushes |
