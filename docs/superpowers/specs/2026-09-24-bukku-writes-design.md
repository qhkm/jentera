...
## Bukku's official API, not the browser

- **Every read and write here goes through Bukku's official REST API**
  (`api.bukku.my`). The owner's API token is held in the Worker's vault,
  and the Worker makes each call.
- **The browser only helps connect.** `runner/src/browser-recipes.mjs`
  reads the token and subdomain from Bukku's integrations page when the
  owner connects by signing in. It creates nothing, and by design it never
  presses Refresh Token.

**Known hole, outside this design.**

- A browser sign-in leaves a live Bukku session in the business browser.
  Hermes's browser tools could then create or change records by clicking
  through Bukku's own pages.
- The approval gate governs API calls, not clicks, so that path would skip
  it.
- This design narrows the hole but does not close it. The
  `propose_business_record` description tells the agent that Bukku
  quotations, invoices, contacts and payments are made only through that
  tool, never in the browser, and `ask.ts` says the same whenever Bukku is
  connected.
- An instruction is a request, not a constraint. Enforcing it means
  refusing browser actions on `*.bukku.my` after sign-in, and that is its
  own design. It is recorded in `docs/todo.md`.
...
| `entity` | contact | `company` or `individual`. Defaults to `company`. Maps to Bukku's `entity_type` `MALAYSIAN_COMPANY` or `MALAYSIAN_INDIVIDUAL`, which MyInvois uses. Foreign entities are out of scope. |
| `email`, `phone` | contact | Optional, and checked for format |

Bukku requires `entity_type`, `legal_name`, `contact_code` and `types` on
a new contact (`bukku-contacts-api.yaml`).

- **`types`** is always `['customer']`.
- **`contact_code`** is generated by the Worker in Bukku's own convention:
  `C-`, then the first letter or digit of the name, then a four-digit
  running number (`C-K0001`). The next number comes after the highest
  existing code with that prefix. A clash reported by Bukku is retried once
  with the next number.

The call sends `{connector: "Bukku", op, args}` (no `runId`; see step 5
below) to
...
4. **Queue.** One `approval` row is written with these fields:
   ...
   sha256 of the resolved request.
   - Under an advisory lock, a row with the same key is returned instead
     of creating a second one, but only if that row is pending or was
     created in the last 10 minutes.
   - The window is what separates an agent's retry from a real repeat
     order, such as the same quotation to the same customer next week.
     Calendar's `requestId` has no such window, because an event is a
     moment in time.
5. **Tell the owners.** Every owner gets an `approval_requested`
   notification, sent at once with `deliverPendingPushes`.
   - There is no run trace. The plugin cannot send a run id: a sprite's
     Hermes gateway passes tool handlers no run, which
     `plugins/jentera/tools.py` explains. So the approval row, with its
     time, decision and result, is the record.
   - If a later Hermes supplies a run id, the route already accepts
     `runId` and would append `action.proposed` with `{connector, op}`
     only.
...
The only actions are **Approve** and **Decline**.

After a decision the card stays on screen with the outcome, for example
"Draft QT-00012 created in Bukku. Open Bukku to finalise it", or the failure
reason. A **Done** button then refreshes the list. `decideApproval` returns
the Worker's outcome so the card can show it. It is not a toast, because the
inbox renders outside any toast provider.
