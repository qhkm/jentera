...
| Item | Why | Done when |
|---|---|---|
| **In-app account deletion** | Apple 5.1.1(v), and their FAQ closes the "we sign in on the web" escape. Google wants an in-app path *and* a web URL | **Shipped 17 Sep** (spec: `docs/superpowers/specs/2026-09-16-account-deletion-design.md`). Deletion is initiated from the account menu in the app and actually deletes: business, RLS-scoped rows, R2 artifacts, `oauth_identity` (via `app_user`'s cascade), email-keyed `platform_access`/`trial_redemption`/`waitlist_entry`, invitations, push rows — and the Fly sprite, seven days later, by cron (`worker/src/account-deletion/purge.ts`). Known gap: `login_token` is keyed by email, not `user_id`, so it is not cascaded and the purge does not clear it separately — see `docs/todo.md`'s 17 Sep entry for that and for the stalled-deletion runbook |
