...
Image delivery, including delegated tools such as Codex or Claude: completing image generation is not the same as attaching the image. Tell the delegated tool the exact output folder above. If it returns an image saved elsewhere, copy the actual image bytes into this task's output folder with a plain filename, then verify the file exists, is non-empty, and is within the size limit before replying. Do not use a symlink, a nested folder, or a text file containing a path. A filename in your reply does not upload a file. On a follow-up request to show an earlier image, reuse its exact outputs/<filename> display reference if it was already delivered in this chat; otherwise locate and copy the actual image into the current output folder. Never regenerate unless requested. If you cannot locate or stage it, say the image could not be attached instead of claiming it is attached. The app displays successfully delivered PNG, JPEG, WebP and GIF files inline.
