...
export async function truncateAll(): Promise<void> {
  await asOwner(async (sql) => {
    await sql.unsafe(`
      truncate notification, routine_change, routine_occurrence, routine, runtime_task_outbox, runtime_usage, runtime_budget, runtime_task, agent_runtime, credential, connection, run_event, run, work_record, goal_checkpoint, goal, business_fact, approval, action_policy, work_done, learn, membership,
               activation_milestone, artifact, chat_session, invitation, push_outbox, push_subscription, reminder, specialist_profile, workspace, workspace_member,
               oauth_identity, native_auth_code, session, login_token, auth_attempt, account_deletion, business, app_user, fmcv_rider_spend, model_call
