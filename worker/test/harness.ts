...
export async function truncateAll(): Promise<void> {
  await asOwner(async (sql) => {
    await sql.unsafe(`
      truncate runtime_spare_monitor, runtime_spare, notification, routine_change, routine_occurrence, routine, runtime_task_outbox, runtime_usage, runtime_budget, runtime_task, agent_runtime, credential, connection, run_event, run, work_record, goal_checkpoint, goal, business_fact, approval, action_policy, work_done, learn, membership,
               oauth_identity, native_auth_code, session, login_token, auth_attempt, business, app_user, fmcv_rider_spend, model_call, account_block, account_identity_block
