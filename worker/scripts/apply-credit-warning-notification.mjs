...
const migration = await readFile(
  new URL('../migrations/071_credit_warning_notification.sql', import.meta.url),
  'utf8',
);
...
const kinds = [
  ...
  'work_needs_you', 'approval_requested', 'reminder_due', 'booking_requested', 'work_finished', 'credit_warning',
];
...
try {
  const verified = await sql.begin(async (tx) => {
    ...
    if (missing.length) {
      throw new Error(`credit_warning migration verification failed: missing ${missing.join(', ')} in ${row.check_def}`);
    }
    return row;
  });
  process.stdout.write(`${JSON.stringify({ ok: true, migration: '071_credit_warning_notification', verified })}\n`);
} finally {
