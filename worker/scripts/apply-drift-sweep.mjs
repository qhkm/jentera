...
const migration = await readFile(
  new URL('../migrations/018_runtime_drift_sweep.sql', import.meta.url),
  'utf8',
);
    ...
    const [row] = await tx`
      select
        (select count(*) from pg_proc
          where proname = 'runtime_drift_targets'
            and prosecdef)::int > 0 as drift_fn,
        (select count(*) > 0 from information_schema.routine_privileges
          where routine_name = 'runtime_drift_targets'
            and grantee = 'aisar_app' and privilege_type = 'EXECUTE') as app_exec`;
    if (!row.drift_fn || !row.app_exec) {
      throw new Error('runtime_drift_targets migration verification failed');
    }
    return row;
  });
  process.stdout.write(`${JSON.stringify({ ok: true, migration: '018_runtime_drift_sweep', verified })}\n`);
} finally {
