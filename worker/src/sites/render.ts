...
import type { BookingPageTheme } from '../apps/bookings/config';
...
/** A private calendar file, returned only after the customer has verified
    their reference and phone number and only for a confirmed booking. */
export function calendarFile(input: Base & { booking: ManagedBooking; generatedAt: Date }): Response {
  const description = `Booking with ${input.businessName}. Reference: ${input.booking.reference}`;
  const lines = [
    'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Jentera//Bookings//EN', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH',
    'BEGIN:VEVENT', `UID:${icsText(input.booking.id)}@book.jentera.ai`, `DTSTAMP:${icsDate(input.generatedAt)}`,
    `DTSTART:${icsDate(input.booking.startsAt)}`, `DTEND:${icsDate(input.booking.endsAt)}`,
    `SUMMARY:${icsText(input.booking.serviceName)}`, `DESCRIPTION:${icsText(description)}`,
    ...(input.location?.trim() ? [`LOCATION:${icsText(input.location.trim())}`] : []),
    'STATUS:CONFIRMED', 'END:VEVENT', 'END:VCALENDAR', '',
  ];
  return new Response(lines.join('\r\n'), { headers: {
    'Content-Type': 'text/calendar; charset=utf-8',
    'Content-Disposition': `attachment; filename="booking-${input.booking.reference}.ics"`,
    ...SECURITY_HEADERS,
  } });
}
...
const T = {
  en: {
    ...
    confirmedHint: 'Your appointment is confirmed.', addGoogleCalendar: 'Add to Google Calendar', downloadCalendar: 'Download calendar file',
  ...
  bm: {
    ...
    note: 'Nota (pilihan)', send: 'Hantar permintaan', back: 'Kembali', otherLang: 'English', poweredBy: 'Dikuasakan oleh Jentera',
    ...
    confirmedHint: 'Janji temu anda telah disahkan.', addGoogleCalendar: 'Tambah ke Google Calendar', downloadCalendar: 'Muat turun fail kalendar',
...
:root{color-scheme:dark;--bg:#080808;--surface:#181818;--ink:#f2f2f2;--muted:#aaa;--line:rgb(255 255 255 / .12);--soft:#242424;--accent:#4aebb5;--accent-ink:#080808;--primary:#f3f7f5;--primary-ink:#101514;--primary-hover:#fff;--primary-hover-ink:#101514;--focus:#72efc5;--error:#ffb4ab;--danger-border:#82483f;--danger-bg:#3a1512;--danger-ink:#ffd9d4;--notice-ink:#f6c8c3;--pending-bg:rgb(249 156 0 / .13);--pending-ink:#ffc45d;--sticky-surface:rgb(24 24 24 / .96);--sticky-shadow:0 8px 24px rgb(0 0 0 / .28);--radius-card:16px;--radius-item:13px;--radius-control:12px}
:root[data-theme="light"]{color-scheme:light;--bg:#f7faf8;--surface:#fff;--ink:#17231f;--muted:#64736e;--line:rgb(18 72 54 / .14);--soft:#edf5f1;--primary:#123a2e;--primary-ink:#fff;--primary-hover:#174b3b;--primary-hover-ink:#fff;--error:#b42318;--danger-border:#dc8f88;--danger-bg:#feeceb;--danger-ink:#8a2c23;--notice-ink:#8a2c23;--pending-bg:#fff4d8;--pending-ink:#8a5a00;--sticky-surface:rgb(255 255 255 / .96);--sticky-shadow:none}
...
.topbar{max-width:1080px;margin:auto;padding:22px 24px 0;display:flex;align-items:center;justify-content:flex-end}.lang{font-size:13px;color:var(--ink);padding:8px 12px;border:1px solid var(--line);border-radius:999px;text-decoration:none}
main{max-width:1032px;margin:28px auto 0;padding:0 24px}.booking-layout{display:grid;grid-template-columns:280px minmax(0,1fr);gap:48px;align-items:start}.business{padding:24px 0}.avatar{width:64px;height:64px;display:grid;place-items:center;overflow:hidden;background:var(--soft);border:1px solid var(--line);border-radius:var(--radius-control);font-size:24px;font-weight:650;margin-bottom:24px}.avatar img{display:block;width:100%;height:100%;object-fit:contain}.eyebrow{font-size:11px;letter-spacing:1.8px;text-transform:uppercase;font-weight:650;color:var(--muted)}.business-name{font-size:27px;line-height:1.25;letter-spacing:-1px;margin:12px 0 18px;overflow-wrap:anywhere}.business-copy{color:var(--muted);font-size:14px}.explanation{border-top:1px solid var(--line);margin-top:30px;padding-top:24px}.explanation strong{font-size:13px}.location{display:flex;gap:10px;margin-top:24px;color:var(--muted);font-size:13px}.location span:first-child{color:var(--accent)}.location strong{display:block;color:var(--ink);font-size:12px}.timezone{font-size:12px;color:var(--muted);margin-top:24px}
.panel{background:transparent;border:1px solid var(--line);border-radius:var(--radius-card);padding:32px;box-shadow:none;min-width:0;min-height:390px}.steps{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));list-style:none;padding:0 0 25px;margin:0 0 30px;border-bottom:1px solid var(--line);gap:10px}.steps li{display:flex;align-items:center;gap:7px;min-width:0;font-size:12px;color:var(--muted);white-space:nowrap}.steps span{display:grid;place-items:center;width:23px;height:23px;flex-shrink:0;border:1px solid var(--line);border-radius:50%;font-size:11px}.steps .done{color:var(--ink)}.steps .done span{border-color:color-mix(in srgb,var(--accent) 55%,transparent);color:var(--accent)}.steps [aria-current]{color:var(--accent);font-weight:650}.steps [aria-current] span{background:var(--accent);border-color:var(--accent);color:var(--accent-ink)}
...
.days{display:flex;gap:8px;overflow-x:auto;padding:4px 4px 12px;scrollbar-width:thin}.day{flex:0 0 74px;padding:9px 5px;border:1px solid var(--line);border-radius:var(--radius-control);text-decoration:none;color:var(--muted);background:var(--surface);text-align:center;font-size:11px}.day strong{display:block;font-size:22px;line-height:1.3;color:var(--ink)}.day[aria-disabled="true"]{opacity:.42}.availability{display:block;margin-top:3px;font-size:9px;color:var(--accent)}.day[aria-disabled="true"] .availability{color:var(--muted)}.day[aria-current="date"]{border-color:var(--accent);background:var(--accent);color:var(--accent-ink);opacity:1}.day[aria-current="date"] strong,.day[aria-current="date"] .availability{color:var(--accent-ink)}.day[aria-disabled="true"][aria-current="date"]{background:var(--soft);color:var(--ink)}.day[aria-disabled="true"][aria-current="date"] strong{color:var(--ink)}.day[aria-disabled="true"][aria-current="date"] .availability{color:var(--muted)}.times{display:grid;grid-template-columns:repeat(auto-fill,minmax(125px,1fr));gap:10px}.times .card{margin:0;min-height:64px;text-align:center;padding:13px 8px}.times .muted{font-size:11px;margin-top:2px}.time-card[aria-current="true"]{border-color:var(--accent);background:color-mix(in srgb,var(--accent) 12%,transparent);box-shadow:0 0 0 1px var(--accent) inset}.time-card[aria-current="true"] .muted{color:var(--ink)}.time-prompt{margin:14px 0 0;text-align:center;font-size:12px}.selection-action{display:flex;align-items:center;justify-content:space-between;gap:18px;margin:22px -8px -8px;padding:14px 8px 8px;background:linear-gradient(to bottom,transparent,var(--bg) 20%)}.selection-action-copy{display:grid;min-width:0}.selection-action-copy span{color:var(--muted);font-size:11px}.selection-action-copy strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.primary-action{display:inline-flex;align-items:center;justify-content:center;gap:8px;min-width:150px;min-height:48px;padding:11px 20px;border:1px solid var(--primary);border-radius:999px;background:var(--primary);color:var(--primary-ink);font-weight:650;text-decoration:none}.primary-action:hover{background:var(--primary-hover);color:var(--primary-hover-ink)}.actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:22px}.actions a,.actions button{display:grid;place-items:center;min-height:48px;margin:0;padding:11px 16px;border-radius:999px;text-align:center;text-decoration:none;font-weight:600}.actions a{border:1px solid var(--line);color:var(--ink)}.actions .receipt-primary{border-color:var(--primary);background:var(--primary);color:var(--primary-ink)}.actions .receipt-primary:hover{background:var(--primary-hover);color:var(--primary-hover-ink)}.danger{border-color:var(--danger-border)!important;background:var(--danger-bg)!important;color:var(--danger-ink)!important}.status{display:inline-flex;padding:5px 10px;border-radius:999px;background:var(--soft);font-size:12px;font-weight:650}.status-pending{background:var(--pending-bg);color:var(--pending-ink)}.status-confirmed{background:color-mix(in srgb,var(--accent) 13%,transparent);color:var(--accent)}.status-declined,.status-cancelled{color:var(--muted)}.booking-time{display:grid;gap:2px;margin-top:14px}.booking-time strong{font-size:17px}.confirmed-hint{margin:14px 0 0;color:var(--accent);font-size:13px}.calendar-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:18px}.calendar-actions a{display:grid;place-items:center;min-height:48px;padding:11px 16px;border:1px solid var(--line);border-radius:999px;color:var(--ink);font-weight:600;text-align:center;text-decoration:none}.calendar-actions .calendar-primary{border-color:var(--primary);background:var(--primary);color:var(--primary-ink)}.calendar-actions .calendar-primary:hover{background:var(--primary-hover);color:var(--primary-hover-ink)}.change-actions{margin-top:12px}.manage-meta{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:18px;padding-top:14px;border-top:1px solid var(--line)}.manage-meta span{display:block}.manage-meta .muted{font-size:11px}.secondary{display:inline-grid;place-items:center;min-height:48px;padding:11px 20px;border:1px solid var(--line);border-radius:999px;color:var(--ink);text-decoration:none}.center{text-align:center}
label{display:block;font-size:13px;font-weight:600;margin:18px 0 7px}input,select,textarea{width:100%;min-height:50px;font:inherit;padding:12px 14px;border:1px solid var(--line);border-radius:var(--radius-control);background:var(--surface);color:var(--ink)}input::placeholder{color:#777}textarea{resize:vertical}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:0 12px}.form-field{min-width:0}.form-submit{margin-top:20px;padding-top:4px}.form-submit button{margin-top:0}.reference-card{padding:20px}.reference-card strong{display:block;margin:3px 0 8px;font-size:24px;letter-spacing:.08em}.reference-card .muted{margin:0}button{width:100%;margin-top:12px;font:inherit;font-weight:600;min-height:48px;padding:12px 24px;border:1px solid var(--primary);border-radius:999px;background:var(--primary);color:var(--primary-ink);cursor:pointer}button:hover{background:var(--primary-hover);color:var(--primary-hover-ink)}form>.muted{font-size:12px;margin-top:20px}.error{color:var(--error);font-size:13px;margin:5px 0 0}.notice{padding:12px 16px;border-radius:10px;background:var(--danger-bg);color:var(--notice-ink)}.success-mark{display:grid;place-items:center;width:56px;height:56px;border-radius:50%;background:var(--soft);color:var(--accent);font-size:26px;margin-bottom:24px}footer{margin:28px 0 32px;font-size:11px;color:var(--muted);text-align:center}footer a{color:inherit;text-decoration:none}footer a:hover{color:var(--accent)}
...
function accentStyle(value?: string): string {
  ...
  const ink = (r * 299 + g * 587 + b * 114) / 1000 >= 150 ? '#080808' : '#ffffff';
  return `:root{--accent:${value.toLowerCase()};--focus:${value.toLowerCase()};--accent-ink:${ink}}`;
}
...
function bookingSummary(input: Base & { booking: ManagedBooking }): string {
  const t = T[input.lang];
  return `<div class="summary"><span class="status status-${input.booking.status}">${t.statuses[input.booking.status]}</span><h2>${escapeHtml(input.booking.serviceName)}</h2>
<div class="booking-time"><strong>${dateText(input.booking.startsAt, input.lang)}</strong><span class="muted">${clockText(input.booking.startsAt, input.lang)} · ${t.timezone}</span></div>
...
function googleCalendarHref(input: Base & { booking: ManagedBooking }): string {
  const params = new URLSearchParams({
    action: 'TEMPLATE', text: input.booking.serviceName,
    dates: `${icsDate(input.booking.startsAt)}/${icsDate(input.booking.endsAt)}`,
    details: `Booking with ${input.businessName}. Reference: ${input.booking.reference}`,
  });
  if (input.location?.trim()) params.set('location', input.location.trim());
  return escapeHtml(`https://calendar.google.com/calendar/render?${params}`);
}
...
export function managePage(input: Base & { token: string; booking: ManagedBooking; confirmCancel?: boolean; notice?: 'cancelled' | 'rescheduled' }): string {
  ...
  if (input.notice) {
    ...
    return layout({ ...branding(input), lang: input.lang, title: cancelled ? t.cancelledTitle : t.rescheduledTitle, businessName: input.businessName, location: input.location,
  ...
  const calendar = input.booking.status === 'confirmed'
    ? `<p class="confirmed-hint">${t.confirmedHint}</p><div class="calendar-actions"><a class="calendar-primary" href="${googleCalendarHref(input)}" target="_blank" rel="noreferrer">${t.addGoogleCalendar}</a><a href="${href(input.slug, `${path}/calendar.ics`, {})}">${t.downloadCalendar}</a></div>`
    : '';
  if (input.confirmCancel && changeable) {
    return layout({ ...branding(input), lang: input.lang, title: t.cancelTitle, businessName: input.businessName, location: input.location,
  ...
  return layout({
    ...
    body: `<h1>${t.manage}</h1>${bookingSummary(input)}${calendar}<p class="muted">${t.changePolicy(input.booking.changeCutoffMinutes)}</p>${changeable
      ? `<div class="actions change-actions"><a href="${href(input.slug, `${path}/reschedule`, { lang: input.lang })}">${t.changeTime}</a><a class="danger" href="${href(input.slug, path, { confirm: 'cancel', lang: input.lang })}">${t.cancelBooking}</a></div>`
...
export function customerMessagePage(input: Base & { kind: 'expired' | 'cutoff' | 'unavailable' }): string {
  const t = T[input.lang];
  return layout({ ...branding(input), lang: input.lang, title: t.manage, businessName: input.businessName, location: input.location,
