...
export function timesPage(input: Base & { service: PublicService; days: DayTimes[]; selected: string; nextAvailable?: string | null; notice: 'taken' | null }): string {
  const t = T[input.lang];
  ...
  const times = chosen && chosen.slots.length > 0
    ...
    : `<div class="empty"><p>${t.noTimes}</p><p class="muted">${t.emptyHint}</p>${input.nextAvailable ? `<a class="next-available" href="${href(input.slug, '', { service: input.service.id, date: input.nextAvailable, lang: input.lang })}">${t.nextAvailable} →</a>` : ''}</div>`;
  return layout({
