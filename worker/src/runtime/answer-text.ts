...
export function answerText(result: unknown): string {
  if (typeof result === 'string' && result.trim()) return result.replace(STEP_STRIP_RE, '').trim().slice(0, 20_000);
  if (result && typeof result === 'object') {
    const text = (result as Record<string, unknown>).text;
    if (typeof text === 'string' && text.trim()) return text.replace(STEP_STRIP_RE, '').trim().slice(0, 20_000);
  }
