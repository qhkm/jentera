      ...
      return <li key={item.id} className={open ? 'connector-option-open' : undefined}>
        <ConnectorCard entry={item} lang={lang} status={status} connected={supported && isConnected(item.id)}>
          {supported && (connections.mode === 'demo' ? <Link className="btn btn-outline" to="/signin">{t('connectors.signin')}</Link>
            : setupMode === 'existing' && connections.mode === 'real' ? <a className="btn btn-outline" href={`#connection-${['telegram', 'google'].includes(item.id) ? item.id : `tokens-${item.id}`}`} onClick={() => {
              const target = document.getElementById(`connection-${['telegram', 'google'].includes(item.id) ? item.id : `tokens-${item.id}`}`);
              const disclosure = target?.closest('details');
              if (disclosure) disclosure.open = true;
            }} aria-label={`${action} ${item.name}`}>{action}</a>
