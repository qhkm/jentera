...
export { seoHead, escapeXml, INDEXABLE_PATHS, INDEXABLE_PAGE_SOURCES, PRIVATE_PATHS, SITE_URL } from './lib/seo';

/** Public pages only. Never render an authenticated provider at build time. */
export function renderPublic(path: string): string {
  if (!['/', '/connect', '/privacy', '/terms', '/404'].includes(path)) throw new Error('Cannot prerender a private route');
